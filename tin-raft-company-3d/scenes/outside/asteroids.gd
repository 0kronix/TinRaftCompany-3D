...
func _spawn_asteroid():
	...
	# Центр — позиция корабля, если задан
	var center = shuttle.global_position if shuttle else Vector3.ZERO
	
	asteroid.global_position = center + Vector3(x, y, z)
	asteroid.spawn_center = center  # Передаём центр астероиду
